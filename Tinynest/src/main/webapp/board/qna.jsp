...
<%
	...
	int printNum=totalQna-(pageNum-1)*pageSize;
...
<div id="board_list">
	<div id="board_title">Q/A</div>

	<% if(loginMember!=null) {//로그인 사용자가 JSP 문서를 요청한 경우 %>
	<div style="text-align: right;">
		<button type="button" onclick="location.href='<%=request.getContextPath()%>/index.jsp?workgroup=board&work=qna_write';">글쓰기</button>
	</div>	
	<% } %>
	<%-- 게시글 목록 출력 --%>
	<table>
		...
		<% } else { %>
			...
			<tr>
				...
				<td class="subject">
					...
					<% if(qna.getRe_qnastep()!=0) {//게시글이 답글인 경우 %>
						<%-- 게시글의 깊이에 의해 왼쪽 여백을 다르게 설정하여 응답되도록 처리 --%>
						<span style="margin-left: <%=qna.getRe_qnalevel()*20%>px;">└ RE: </span>
					<% } %>
					
					<%-- 게시글 상태를 구분하여 제목과 링크를 다르게 설정하여 응답되도록 처리 --%>
					<% if(qna.getQ_status()==1) {//일반 게시글인 경우 %>
						<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=qna_detail&q_no=<%=qna.getQ_no() %>&pageNum=<%=pageNum%>&search=<%=search%>&keyword=<%=keyword%>">
							<%=qna.getQ_title() %>
						</a>
					<% } else if(qna.getQ_status()==2) {//비밀 게시글인 경우 %>
						<span class="secret">비밀글</span>
						<%-- 로그인 사용자가 작성자이거나 관리자인 경우 --%>
						<% if(loginMember!=null && (loginMember.getId().equals(qna.getQ_id()) 
						...
						<% } %>		
					<% } else if(qna.getQ_status()==0) {//삭제 게시글인 경우 %>
						<span class="remove">삭제글</span>
						작성자 또는 관리자에 의해 삭제된 게시글입니다.	
					<% } %>
				</td>
				
				<% if(qna.getQ_status()!=0) {//삭제 게시글이 아닌 경우 %>
				<%-- 작성자 --%>
				...
				아닌 게시글은 날짜와 시간 출력 --%>
				<td>
					<% if(currentDate.equals(qna.getQ_date().substring(0,10))) {//오늘 작성된 게시글인 경우 %>
						<%=qna.getQ_date().substring(11) %>
	...
	<% if(startPage>blockSize) {//첫번째 페이지 블럭이 아닌 경우%>
		<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=qna&pageNum=<%=startPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>">&nbsp;〈&nbsp;</a>
	<% } %>
	
	<% for(int i=startPage;i<=endPage;i++) { %>
		<% if(pageNum!=i) {//요청 페이지 번호와 이벤트 페이지 번호가 다른 경우 - 링크 제공 %>
			<a href="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=qna&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>">&nbsp;<%=i %>&nbsp;</a>
	...
	<form action="<%=request.getContextPath()%>/index.jsp?workgroup=board&work=qna" method="post" class="searchForm">
