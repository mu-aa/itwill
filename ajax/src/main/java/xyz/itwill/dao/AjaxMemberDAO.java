...
public class AjaxMemberDAO extends JdbcDAO {
	...
	public int insertAjaxMember(AjaxMemberDTO ajaxMember) {
		...
		try {
			...
			System.out.println("[ERROR]insertAjaxMember() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
	...
	public AjaxMemberDTO selectAjaxMember(String id) {
		...
		try {
			...
			System.out.println("[ERROR]selectAjaxMember() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return ajaxMember;
	...
	/*
	//이름과 이메일을 전달받아 AJAX_MEMBER 테이블에서 해당 이름과 이메일을 사용하는 회원의
	//아이디를 검색하여 반환하는 메소드
	public String selectAjaxMemberId(String name, String email) {
		...
		try {
			...
			if(rs.next()) {
				//id=rs.getString(1); //첫번째 검색대상(어차피 검색대상은 id 하나뿐)
				id=rs.getString("id");
			}
		} catch (SQLException e) {
			System.out.println("[ERROR]selectAjaxMemberId() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return id;
	...
	//회원정보(이름과 이메일)를 전달받아 AJAX_MEMBER 테이블에서 해당 이름과 이메일을 사용하는
