...
<body>
	<h1>Core - 흐름 제어 태그</h1>
	<hr>
	<%-- if 태그 : 조건에 대한 결과에 따라 태그내용의 포함 여부를 선택하는 태그 - Java if문과 같음 --%>
	<%-- test 속성 : false 또는 true 중 하나를 속성값으로 설정 --%>
	<%-- ㄴ test 속성값이 [false]인 경우 태그내용 미포함, [true]인 경우 태그내용 포함 --%>
	<%-- ㄴ test 속성값으로 EL 연산자를 사용하여 false 또는 true 중 하나를 제공받아 사용 가능 --%>
	<c:if test="true">
		<p>test 속성값이 [true]인 경우 태그 내용 출력-1</p>
	</c:if>

	<c:if test="false">
		<p>test 속성값이 [false]인 경우 태그 내용 미출력-2</p>
	</c:if>
	
	<c:set var="sw" value="true" />
	<%-- test 속성값으로 EL로 제공받은 scope 속성값 사용 가능 --%>
	<c:if test="${sw }">
		<p>test 속성값이 [true]인 경우 태그 내용 출력-3</p>
	</c:if>
	<hr>
	<c:set var="num" value="10" />
	<%-- test 속성값으로 EL 사용 시 EL 표현식에서 EL 연산자를 사용하여 false 또는 true 중
	 하나를 제공받아 사용 가능 --%>
