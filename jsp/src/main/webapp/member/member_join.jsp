...
<%-- ㄴ [회원가입]을 클릭한 경우 회원정보 삽입페이지(member_join_action.jsp)로 이동 - 입력값 전달 --%>
<%-- ㄴ [아이디 중복검사]를 클릭한 경우 팝업창을 실행하여 아이디 중복 검사페이지(id_check.jsp)를 요청 - 아이디 전달 --%>
<%-- ㄴ [우편번호 검색]을 클릭한 경우 팝업창을 실행하여 우편번호 검색페이지(post_search.jsp)를 요청 --%>
<style type="text/css">
fieldset {
	text-align: left;
	margin: 10px auto;
...
</style><%-- 템플릿 페이지라 모든 요청은 index에게 하지만 workgroup, work 값을 전달해 다른 JSP 문서로 스레드 이동 --%>
<form id="join" action="index.jsp?workgroup=member&work=member_join_action" method="post">
<%--아이디 사용 여부를 구분할 값을 저장하는 입력태그 --%>
<%-- ㄴ 0 : 아이디 중복검사 미실행 - 아이디 사용 불가능 --%>
<%-- ㄴ 1 : 아이디 중복검사 실행 - 아이디 사용 불가능 --%>
...
<fieldset>
	...
	<ul>
		<li>
			...
			<div id="idCheckMsg" class="error">아이디 중복검사를 반드시 실행해주세요.</div>
		</li>
		<li>
		...
		<li>
			...
			<input type="text" name="address1" id="address1" size="50" readonly>
...
$("#postSearch").click(function() {
	...
	window.open("<%=request.getContextPath()%>/member/post_search.jsp","postsearch"
		, "width=550, height=600, top=250");
});
--%>
</script>

<%-- 다음에서 제공한 코드를 이용 - 테이블, DTO, DAO 등 만들 필요 없음 --%>
