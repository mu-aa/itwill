...
public class Utility {
	...
	public static String encrypt(String source) {
		...
		try {
			//MessageDigest 객체 : 암호화 처리 기능 제공
			//MessageDigest.getInstance(String algorithm) : 매개변수에 전달된 암호화 알고리즘이 저장된 
			//MessageDigest 객체를 생성하여 반환하는 메소드
			//ㄴ 매개변수에 잘못된 암호화 알고리즘을 전달할 경우 NoSuchAlgorithmException 발생
			//단방향 암호화 알고리즘 : MD5(사용금지), SHA-1, SHA-256(이상버전 권장), SHA-512
			//양방향 암호화 알고리즘 : AES-123, RSA 등
			MessageDigest messageDigest=MessageDigest.getInstance("SHA-256");
			
			//MessageDigest.update(byte[] input) : MessageDigest 객체에 암호화 처리하기 위한 값(byte 배열)을
			//전달받아 저장하기 위한 메소드
			//String.getBytes() : String 객체에 저장된 문자열을 원시데이터(Byte 배열)로 변환하여 반환하는 메소드
			messageDigest.update(source.getBytes());
			
			//MessageDigest.digest() : MessageDigest 객체에 저장된 암호화 알고리즘으로 원시데이터를 암호화 처리하여
			//byte 배열로 반환하는 메소드
			...
			System.out.println("[ERROR]잘못된 암호화 알고리즘을 사용했습니다.");
		}
		return password;
	}
	...
	public static String stripTag(String source) {
		...
		//Pattern.matcher(CharSequence input) : Pattern 객체에 저장된 정규표현식과
		//입력값이 저장된 matcher 객체를 반환하는 메소드
		//Matcher 객체 : 정규표현식과 입력값을 비교 처리하기 위한 기능을 제공하는 객체
		//Matcher.replaceAll(String replacement) : 정규표헌식과 입력값을 비교하여 패턴이 동일한 문자열을 모두 찾아 
		//매개변수로 전달받은 문자열로 변경하는 메소드
		source=htmlTag.matcher(source).replaceAll("");
		
		return source;
	}
	
	//문자열을 전달받아 태그 관련 문자열을 회피문자로 변경하여 반환하는 메소드
