...
public class MemberDAO extends JdbcDAO {
	...
	public List<MemberDTO> selectMemberList() {
		...
		try {
			...
			System.out.println("[ERROR]selectMemberList() 메소드 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return memberList;
	...
	//아이디를 전달받아 MEMBER 테이블에 저장된 해당 아이디의 회원정보를 삭제하고 삭제행의 수를 반환하는 메소드 - 공지사항, 장바구니 등에 활용
	public int deleteMember(String id) {
		...
		try {
			...
			System.out.println("[ERROR]deleteMember() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt);
		}
		return rows;
